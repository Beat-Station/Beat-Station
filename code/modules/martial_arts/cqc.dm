...
/mob/living/carbon/human/proc/cqc_help()
	...
	to_chat(usr, "<b><i>You remember the training from your former mentor...</i></b>")
	to_chat(usr, "<span class='notice'>Roundhouse Kick</span>: Punch 5 times. Knocks the opponent unconscious.")
	to_chat(usr, "<span class='notice'>Robust Disarm</span>: Your disarms have no push chance, however, when you disarm someone the weapon is instantly put in your hands.")
	to_chat(usr, "<span class='notice'>Clinch</span>: Grab. 100% chance to instantly grab your opponent into aggressive. Can be reinforced into neckgrab quickly.")
	to_chat(usr, "<span class='notice'>Half-wing Choke</span>: Replaces normal choking. Faster to perform, also gives 20 staminaloss to opponent on attempted choke.")
	to_chat(usr, "<span class='notice'>Legsweep</span>: Harm intent w/ AGGRESSIVE grab in active hand. Cannot be performed on downed opponent. Sweeps the opponent off their feet for a while. Useful if you failed a clinch.")
	to_chat(usr, "<span class='notice'>Karate Chop</span>: Disarm intent w/ NECK grab in active hand. Cannot be performed on downed opponent. Makes target dizzy, gives them 20 staminaloss.")
	to_chat(usr, "<span class='notice'>Face Slam</span>: Harm intent w/ NECK grab in active hand. Cannot be performed on downed opponent. Slams opponent into the ground, knocking them unconscious. Only reccomended in 1v1 fights.")
	to_chat(usr, "<i>The arts of CQC are most effective as a surprise tactic. They were designed as a martial art that would aid a stealthy approach. Fighting multiple enemies with it would prove difficult.</i>")
...
/datum/martial_art/cqc/teach(mob/living/carbon/human/H, make_temporary=0)
	..()
	to_chat(H, "<span class='userdanger'>You know the basics of CQC!</span>")
	to_chat(H, "<span class='danger'>Recall your teachings using the Recall CQC Training verb in the Martial Arts menu, in your verbs menu.</span>")
	H.verbs += /mob/living/carbon/human/proc/cqc_help

/datum/martial_art/cqc/remove(mob/living/carbon/human/H)
	..()
	to_chat(H, "<span class='userdanger'>You forget the basics of CQC..</span>")
	H.verbs -= /mob/living/carbon/human/proc/cqc_help
...
/datum/martial_art/cqc/grab_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(A == D)
		return 1 //You shouldn't be able to attack yourself
	D.grabbedby(A, 1)
