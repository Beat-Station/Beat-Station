...
/datum/data/pda/app/supply/update_ui(mob/user as mob, list/data)
	...
	for(var/S in shuttle_master.shoppinglist)
		...
		supplyOrderCount++
		supplyOrderData[++supplyOrderData.len] = list("Number" = SO.ordernum, "Name" = html_encode(SO.pack.name), "ApprovedBy" = SO.orderer, "Comment" = html_encode(SO.comment))
	...
	for(var/S in shuttle_master.requestlist)
		var/datum/supply_order/SO = S
		requestCount++
		requestData[++requestData.len] = list("Number" = SO.ordernum, "Name" = html_encode(SO.pack.name), "OrderedBy" = SO.orderer, "Comment" = html_encode(SO.comment))
