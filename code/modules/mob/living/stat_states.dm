...
/mob/living/proc/KnockOut(updating = 1)
	if(stat == DEAD)
		...
		return 0
	add_logs(src, null, "fallen unconscious at [atom_loc_line(get_turf(src))]", admin=1, print_attack_log = 0)
	stat = UNCONSCIOUS
	if(updating)
...
/mob/living/proc/WakeUp(updating = 1)
	if(stat == DEAD)
		...
		return 0
	add_logs(src, null, "woken up at [atom_loc_line(get_turf(src))]", admin=1, print_attack_log = 0)
	stat = CONSCIOUS
	if(updating)
		// update_blind_effects()
		update_canmove()
